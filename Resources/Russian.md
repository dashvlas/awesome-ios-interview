...
`viewWillAppear` — вью будет добавлено в иерархию, определены границы bounds вью, но ориентация экрана не определена.   Вызывается каждый раз, когда появляется вью. Не добавляйте сюда код, который должен быть выполнен только один раз!
...
- `Быстрая`: 
	Realm — невероятно быстрая библиотека для работы с базой данных. Realm быстрее, чем SQLite и CoreData(ORM-обертка над 	   SQLite), и сравнительные тесты здесь — лучшее доказательство для этого.
- `Кросс-платформенная`: 
