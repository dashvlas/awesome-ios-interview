- [UIKit](#uikit)
	- [Чем отличаются bounds и frame?](#bounds-frame)   
	- [Как работает UITableView?](#uitableView-works)   
	- [Разница между points и pixels?](#points-pixels)   
	- [Как поймать неприятный лаг? На что обращать внимание?](#weird-lag)   
	- [Autolayout](#autolayout)   
- [Testing](#testing)   
	- [Unit-тесты](#unit-тесты)   
	- [TDD](#tdd)   
- [Tasks](#tasks)   
	- [Какой метод вызовется: класса A или класса B?](#method-called)
	- [Что выведется в консоль?](#console-log)   
	- [Как это сделать повторяемый таймер в бекграунде?](#background-timer)   
	- [Что произойдет после запуска приложения?](#launch-app)   
- [SDK](#sdk)   
	- [Какие существуют root классы в iOS? Для чего нужны root классы?](#classes-root)  
	- [NSCoding, archiving](#coding)  
	- [Как работают push-уведомления?](#pushnotifications)  
	- [Memory warning](#memorywarning)    
- [Patterns](#patterns)   
	- [Observer](#observer)   
	- [Chain of responsibility](#chain-of-responsibility)   
	- [Singleton](#singleton)   
	- [Способы реализации паттерна Observer](#observer)   
	- [Factory Method](#factorymethod)   
	- [MVC. Чем отличается пассивная модель от активной?](#mvc)   
        - [Минусы singleton](#singletonminuses)
	- [Responder chain](#responderchain)
	- [Lazy initialization](#lazyinitialization)  
- [OOP](#oop)  
	- [Что такое SOLID?](#solid)
	- [Виртуальный метод](#virtualmethod)
	- [Что такое нарушение инкапсуляции?](#incapsulation-offence)
- [Language](#language)
	- [Различия isKindOfClass и isMemberOfClass](#runtime-class)
	- [Когда лучше использовать категорию, а когда наследование?](#category-inheritance)
	- [Из чего состоит NSError?](#nserror?)
	- [KVC](#kvc)
	- [Fast enumeration](#fast-enumeration)
	- [Чем объект Objective-C отличается от структуры С?](#c-structure-vs-objective-c-object)
	- [Что такое runLoop, когда он используется?](#runloop)
	- [nil, Nil, NULL, NSNull](#nil)
	- [Формальные и неформальные протоколы](#in\formal-protocols)
	- [Что такое `@dynamic`?](#dynamic)
	- [Тип id](#id)
	- [Понятия ОО-программирования в Objective-C](#objective-c-oop)
	- [Как работает механизм сообщений?](#message-mechanism)
	- [Приватные методы в Objective-C?](#private-methods)
	- [Можно ли добавить ivar в категорию?](#ivar-category)
	- [Garbage Collector](#garbage-collector)
	- [ARC](#arc)
	- [Освобождение памяти в MRC](#mrc-release)
	- [Указатель isa](#isa)
	- [В чем отличие void* от id?](#void-id)
	- [KVO](#kvo)
	- [`Atomic` vs `nonatomic`. Переопределение сеттера в MRC?](#non\atomic)
	- [Чем отличается include от import?](#include-import)
	- [Что такое селектор (selector)? Как его вызвать?](#selector)
- [GENERAL](#general)
	- [Архитектура REST](#rest)
	...
	- [Чем абстрактный класс отличается от интерфейса?](#abstract-class-interface)
	- [Жизненный цикл ViewController](#viewcontroller-lifecycle)
	- [Какой контент лучше хранить в Documents, а какой в Cache?](#documents-cache)
	- [Какие различия между HEAD, GET, POST, PUT?](#head-get-post-put)
- [БД](#DB)
	- [Какие типы хранилищ поддерживает CoreData?](#coredata-store-support)
	- [Что такое ленивая загрузка? Что ее связывает с Core Data?](#lazy-coredata-load)
	- [Целесообразность использования CoreData](#why-to-use-coredata)
	- [SQLite](#sqlite)
	- [Что такое NSManagedObjectId?](#nsmanagedobjectid?)
	- [NSFetchedResultsController](#nsfetchedresultscontroller)
	- [Что такое контекст (Managed object context)?](#managed-object-context)
	- [Что такое Persistent store coordinator?](#persistent-store-coordinator)
	- [Какие есть нюансы при использовании Core Data в разных потоках?](#coredata-in-different-threads)
- [CONCURRENCY](#concurrency)
	- [Runloop](#runloop)
	- [Способы достижения многопоточности в iOS и mac OS](#concurrency-apple-platforms)
	- [Что такое deadlock?](#deadlock)
	- [Что такое livelock?](#livelock)
	- [Что такое DispatchGroup?](#dispatchgroup)
	- [Разница между синхронными и асинхронными задачами](#a\sync-tasks)
	- [Зачем использовать synchronized?](#synchronized)
	- [Что такое мьютекс (mutex)?](#mutex)
	- [Что такое семафор (semafor)?](#semafor)
# UIKit

<a name="bounds-frame"></a>
...
<a name="incapsulation-offence"></a>
...
```
<a name="c-structure-vs-objective-c-object"></a>
...
<a name="designated-initializer"></a>
...
<a name="abstract-class-interface"></a>
...
<a name="viewcontroller-lifecycle"></a>
...
<a name="coredata-store-support"></a>
...
<a name="nsfetchedresultscontroller"></a>
...
<a name="managed-object-context"></a>
...
<a name="persistent-store-coordinator"></a>
...
<a name="coredata-in-different-threads"></a>
...
<a name="concurrency-apple-platforms"></a>
## Способы достижения многопоточности в iOS и mac OS
